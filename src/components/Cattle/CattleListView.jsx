...
export function CattleListView({ 
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="custom-card p-3 sm:p-3.5 flex flex-col xl:flex-row xl:items-center justify-between gap-3 shadow-sm border border-slate-200/80 dark:border-slate-800">
        
        {/* Indicadores Resumen a la Izquierda */}
        <div className="flex items-center gap-2 text-xs flex-wrap">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-extrabold text-slate-800 dark:text-white bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm whitespace-nowrap">
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black bg-emerald-100/90 text-emerald-800 dark:bg-emerald-950/80 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700 shadow-sm whitespace-nowrap">
            🟢 <strong>{summary.activeCount}</strong> activas / en finca
          </span>

          {summary.soldCount > 0 && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black bg-amber-100/90 text-amber-900 dark:bg-amber-950/80 dark:text-amber-300 border border-amber-300 dark:border-amber-700 shadow-sm whitespace-nowrap">
              🏷️ <strong>{summary.soldCount}</strong> vendidas
            </span>
          )}

          {summary.deadCount > 0 && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-rose-100/90 text-rose-800 dark:bg-rose-950/80 dark:text-rose-300 border border-rose-300 dark:border-rose-700 shadow-sm whitespace-nowrap">
              💀 <strong>{summary.deadCount}</strong> bajas
            </span>
          )}

          <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 font-semibold text-xs border border-slate-200/80 dark:border-slate-700 shadow-sm whitespace-nowrap">
            Biomasa: <strong className="text-emerald-600 dark:text-emerald-400 font-black">{formatNumber(summary.totalKg, 0)} kg</strong>
          </span>

          <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-blue-50/90 dark:bg-blue-950/50 text-slate-700 dark:text-slate-300 font-semibold text-xs border border-blue-200 dark:border-blue-800 shadow-sm whitespace-nowrap">
            Utilidad: <strong className="text-blue-600 dark:text-blue-400 font-black">{formatCurrency(summary.totalProfit)}</strong>
          </span>
        </div>

        {/* Botones de Acción a la Derecha */}
        <div className="flex items-center gap-2 shrink-0 flex-nowrap overflow-x-auto self-start xl:self-auto">
          {/* Botón Liquidar Lote / Compañía */}
          {onOpenPartnershipModal && (
            <button
              onClick={onOpenPartnershipModal}
              className="px-3 py-2 rounded-xl bg-teal-50 hover:bg-teal-100 dark:bg-teal-950/60 dark:hover:bg-teal-900/60 text-teal-800 dark:text-teal-200 font-bold text-xs flex items-center gap-1.5 transition border border-teal-300 dark:border-teal-700 shadow-sm cursor-pointer whitespace-nowrap min-h-[38px]"
          ...
          {onOpenGlossary && (
            <button
              ...
              className="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs flex items-center gap-1.5 transition border border-slate-200 dark:border-slate-700 shadow-sm cursor-pointer whitespace-nowrap min-h-[38px]"
          ...
          <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm shrink-0 min-h-[38px]">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-1.5 rounded-lg transition min-w-[30px] flex items-center justify-center cursor-pointer ${viewMode === 'grid' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition min-w-[30px] flex items-center justify-center cursor-pointer ${viewMode === 'table' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800 dark:hover:text-white'}`}
          ...
          {onOpenBatchEntry && (
            <button
              ...
              className="px-3.5 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs flex items-center gap-1.5 shadow-md shadow-emerald-600/20 transition cursor-pointer whitespace-nowrap min-h-[38px]"
