...
export function VaccinationRecordModal({
  ...
  const [targetType, setTargetType] = useState('all'); // 'all' | 'batch' | 'young_females' | 'individual'
  ...
  const coveredCount = useMemo(() => {
    ...
    return 0;
  }, [targetType, activeCattle, youngFemales, selectedBatch, selectedCattleId]);
  ...
  const handleSubmit = (e) => {
    ...
    if (targetType === 'batch') targetLabel = `Lote: ${selectedBatch || 'Sin lote'}`;
    if (targetType === 'young_females') targetLabel = 'Hembras jóvenes (Brucelosis 3-9m)';
  ...
  return (
    <div className={`fixed inset-0 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 ${zIndex} overflow-y-auto animate-fadeIn`}>
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-2xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-auto">
        ...
        {/* Encabezado */}
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-emerald-50 via-teal-50 to-white dark:from-emerald-950/30 dark:via-slate-900 dark:to-slate-900">
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 overflow-y-auto space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 flex items-center justify-between">
              ...
              <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950 px-2 py-0.5 rounded-lg border border-emerald-200 dark:border-emerald-800">
                {coveredCount} cabezas cubiertas
              </span>
            </label>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
              <button
                ...
                className={`p-2.5 rounded-xl border text-center transition cursor-pointer ${
                  targetType === 'all'
                    ...
                    : 'bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold border-slate-200 dark:border-slate-700'
                }`}
              >
              ...
              >
                ...
                <span className="text-xs block leading-tight">Lote / Potrero</span>
                <span className="text-[10px] opacity-75 font-semibold block">Específico</span>
              </button>
                ...
                >
                  ...
                  {activeCattle.map(c => (
                    <option key={c.id} value={c.id}>
                      Chapa: {c.tagNumber} {c.name ? `(${c.name})` : ''} • {c.category || 'Bovino'}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  placeholder="Ej. Todo el ganado vacunado sin novedad..."
                  value={notes}
          ...
          <div className="pt-2 flex items-center justify-end gap-3 border-t border-slate-200 dark:border-slate-800">
            ...
            >
              ...
              <span>Guardar Registro de Vacunación</span>
            </button>
          </div>
